...
@section('scripts')
    <script type="text/javascript">
        ...
        async function getPaymentListData(val) {
            ...
            if (currencyAtRight) {
                ...
                if (data.remaining_val < 0) {
                    ...
                    data.remaining_val = '(-' + parseFloat(remaining_val).toFixed(decimal_degits) + "" +
                        currentCurrency + ')';
                } else {
                ...
                if (data.remaining_val < 0) {
                    ...
                    data.remaining_val = '(-' + currentCurrency + "" + parseFloat(remaining_val).toFixed(
                        decimal_degits) + ')';
        ...
        function prev() {
            ...
            if (end != undefined || end != null) {
                ...
                if (jQuery("#selected_search").val() == 'restaurant' && jQuery("#search").val().trim() != '') {
                    listener = ref.orderBy('title').limit(pagesize).startAt(jQuery("#search").val()).endAt(jQuery("#search")
                        .val() + '\uf8ff').startAt(end).get();
        ...
        async function remainingPrice(vendorID) {
            ...
            await database.collection('payouts').where('vendorID', '==', vendorID).where('paymentStatus', '==',
                'Success').get().then(async function(payoutSnapshots) {
                ...
                await database.collection('users').where('vendorID', '==', vendorID).get().then(
                    async function(vendorSnapshots) {
                        var vendor = [];
                        var wallet_amount = 0;
        ...
        async function remainingPriceOLD(vendorID) {
            ...
            await database.collection('payouts').where('vendorID', '==', vendorID).get().then(async function(
                payoutSnapshots) {
                ...
                await database.collection('restaurant_orders').where('vendor.id', '==', vendorID).where(
                    "status", "in", ["Order Completed"]).get().then(async function(orderSnapshots) {
                    ...
                    orderSnapshots.docs.forEach((order) => {
                        ...
                        orderData.products.forEach((product) => {
                            if (product.price && product.quantity != 0) {
                                ...
                                if (product.extras_price != undefined) {
                                    extras_price = parseFloat(product
                                        .extras_price) * parseInt(product
                                        .quantity);
                                }
                                var productTotal = parseFloat(product.price) *
                                    parseInt(product.quantity) + extras_price;
                        ...
                        if (!isNaN(tax)) {
                            mainproductTotal = parseFloat(mainproductTotal) +
                                parseFloat(tax);
                        }

                        if (orderData.adminCommission != undefined && orderData
                            .adminCommissionType != undefined && orderData
                            .adminCommission > 0 && mainproductTotal > 0) {
                            var commission = 0;
                            if (orderData.adminCommissionType == "Percent") {
                                commission = (mainproductTotal * parseFloat(orderData
                                    .adminCommission)) / 100;
                        ...
                        } else if (orderData.adminCommission != undefined && orderData
                            .adminCommission > 0 && mainproductTotal > 0) {
                    ...
                    if (currencyAtRight) {
                        ...
                        total_price_val = parseFloat(total_price).toFixed(decimal_degits) + "" +
                            currentCurrency;

                        paid_price_val = parseFloat(paid_price).toFixed(decimal_degits) + "" +
                            currentCurrency;

                        remaining_val = parseFloat(remaining).toFixed(decimal_degits) + "" +
                            currentCurrency;

                    } else {

                        total_price_val = currentCurrency + "" + parseFloat(total_price)
                            .toFixed(decimal_degits);

                        paid_price_val = currentCurrency + "" + parseFloat(paid_price).toFixed(
                            decimal_degits);

                        remaining_val = currentCurrency + "" + parseFloat(remaining).toFixed(
                            decimal_degits);

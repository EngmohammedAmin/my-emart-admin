    ...
    @section('scripts')
        <script type="text/javascript">
            ...
            async function getListData(val) {
                ...
                html = html + '<td class="action-btn"><a id="' + val.id + '" name="vendor_view" data-auth="' + val
                    .vendorID +
                    '" href="javascript:void(0)" data-toggle="modal" data-target="#bankdetailsModal"><i class="fa fa-eye"></i></a><a id="' +
                    val.id + '" name="vendor_check" data-auth="' + val.vendorID + '" amount="' + price_val +
                    '" href="javascript:void(0)"><i class="fa fa-check" style="color:green"></i></a><a id="' + val.id +
                    '" data-price="' + price + '" amount="' + price_val + '" name="reject-request" data-auth="' + val
                    .vendorID + '" href="javascript:void(0)"><i class="fa fa-close" ></i></a></td>';
                            ...
                            database.collection('payouts').doc(id).update({
                                ...
                                database.collection('users').doc(vendor.id).update({
                                    ...
                                    if (user && user != undefined) {
                                        ...
                                        var emailData = await sendMailToRestaurant(user,
                                            id, 'Disapproved', amount);
            ...
            function prev() {
                ...
                if (end != undefined || end != null) {
                    ...
                    if (jQuery("#selected_search").val() == 'note' && jQuery("#search").val().trim() != '') {
                        listener = refData.orderBy('note').limit(pagesize).startAt(jQuery("#search").val()).endAt(jQuery(
            ...
            function searchtext() {
                ...
                if (jQuery("#selected_search").val() == 'note' && jQuery("#search").val().trim() != '') {
                    ...
                    wherequery = refData.orderBy('note').limit(pagesize).startAt(jQuery("#search").val()).endAt(jQuery(
                        "#search").val() + '\uf8ff').get();

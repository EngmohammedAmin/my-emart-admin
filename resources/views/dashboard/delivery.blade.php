...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            db.collection('vendor_orders').where('status', 'in', ["Order Placed"]).where('vendor.section_id', '==',
                active_id).get().then(
                (snapshot) => {
            ...
            db.collection('vendor_orders').where('status', 'in', ["Order Accepted", "Driver Accepted"]).where(
                'vendor.section_id', '==', active_id).get().then(
                (snapshot) => {
                    jQuery("#confirmed_count").empty();
                    jQuery("#confirmed_count").text(snapshot.docs.length);
                });

            db.collection('vendor_orders').where('status', 'in', ["Order Shipped", "In Transit"]).where(
                'vendor.section_id', '==', active_id).get().then(
                (snapshot) => {
            ...
            db.collection('vendor_orders').where('status', 'in', ["Order Completed"]).where('vendor.section_id',
                '==', active_id).get().then(
                (snapshot) => {
            ...
            db.collection('vendor_orders').where('status', 'in', ["Order Rejected"]).where('vendor.section_id',
                '==', active_id).get().then(
                (snapshot) => {
            ...
            db.collection('vendor_orders').where('status', 'in', ["Driver Rejected"]).where('vendor.section_id',
                '==', active_id).get().then(
                (snapshot) => {
            ...
            db.collection('vendor_orders').where('status', 'in', ["Driver Pending"]).where('vendor.section_id',
                '==', active_id).get().then(
                (snapshot) => {
        ...
        async function getTotalEarnings() {
            ...
            await db.collection('vendor_orders').where('status', 'in', ["Order Completed"]).where('vendor.section_id',
                '==', active_id).get().then(async function(orderSnapshots) {
                ...
                paymentData.forEach((order) => {
                    ...
                    orderData.products.forEach((product) => {
                        ...
                        if (product.price && product.quantity != 0) {
                            var extras_price = 0;
                            if (product.extras_price != undefined && product.extras_price !=
                                null) {
                                extras_price = parseFloat(product.extras_price) * parseInt(
                                    product.quantity);
                            }
                            if (!isNaN(extras_price)) {
                                var productTotal = (parseFloat(product.price) * parseInt(
                                    product.quantity)) + extras_price;
                            } else {
                                var productTotal = (parseFloat(product.price) * parseInt(
                                    product.quantity));
                            }
        ...
        function buildOrderHTML(snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                html = html + '<td data-url="' + vendorroute + '" class="redirecttopage">' + val.vendor.title +
                    '</td>';

                var price = buildHTMLProductstotal(val);

                html = html + '<td data-url="' + route + '" class="redirecttopage">' + price + '</td>';
                html = html + '<td data-url="' + route +
                    '" class="redirecttopage"><i class="fa fa-shopping-cart"></i> ' + val.products.length + '</td>';
                html = html + '</a></tr>';
                count++;
            });
            return html;
        ...
        function setVisitors() {
            ...
            const data = {
                ...
                datasets: [{
                    data: [jQuery("#vendor_count").text(), jQuery("#order_count").text(), jQuery(
                            "#product_count").text(), jQuery("#users_count").text(), jQuery("#driver_count")
                        .text()
                    ],
                    backgroundColor: [
                        '#218be1',

            ...
            <div class="card mb-3 business-analytics">
                ...
                <div class="card-body">
                    ...
                    <div class="row business-analytics_list">
                        ...
                        <div class="col-sm-6 col-lg-3">
                            <a class="order-status confirmed" href="{!! route('parcel_orders', 'status=order-confirmed') !!}">
                                <div class="data">
                                    <i class="mdi mdi-check-circle"></i>
                                    <h6 class="status">{{ trans('lang.dashboard_order_confirmed') }}</h6>
                                </div>
                                <span class="count" id="confirmed_count"></span>
                            </a>
                        </div>

                        <div class="col-sm-6 col-lg-3">
                            <a class="order-status packaging" href="{!! route('parcel_orders', 'status=order-shipped') !!}">
                                <div class="data">
                        ...
                        <div class="col-sm-6 col-lg-3">
                            <a class="order-status delivered" href="{!! route('parcel_orders', 'status=order-completed') !!}">
                                <div class="data">
                        ...
                        <div class="col-sm-6 col-lg-3">
                            <a class="order-status canceled" href="{!! route('parcel_orders', 'status=order-canceled') !!}">
                                <div class="data">
...
@section('scripts')
    ...
    <script>
        ...
        async function getTotalEarnings() {
            ...
            await db.collection('parcel_orders').where('status', 'in', ["Order Completed"]).get().then(async function(
                orderSnapshots) {
        ...
        function buildDriverHTML(snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                html = html + '<td data-url="' + driverView + '" class="redirecttopage">' + val.firstName + ' ' +
                    val.lastName + '</td>';
                html = html + '<td data-url="' + driverroute + '" class="redirecttopage">' + val.orderCompleted +
                    '</td>';
        ...
        function buildOrderHTML(snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                html = html + '<td data-url="' + user_view + '" class="redirecttopage">' + val.author.firstName +
                    ' ' + val.author.lastName + '</td>';

                var price = buildParcelTotal(val);

                html = html + '<td data-url="' + route + '" class="redirecttopage">' + price + '</td>';
                if (val.status == 'Order Placed') {
                    html = html + '<td data-url="' + route + '" class="redirecttopage order_placed"><span>' + val
                        .status + '</span></td>';

                } else if (val.status == 'Order Accepted') {
                    html = html + '<td data-url="' + route + '" class="redirecttopage order_accepted"><span>' + val
                        .status + '</span></td>';

                } else if (val.status == 'Order Rejected') {
                    html = html + '<td data-url="' + route + '" class="redirecttopage order_rejected"><span>' + val
                        .status + '</span></td>';

                } else if (val.status == 'Driver Pending') {
                    html = html + '<td data-url="' + route + '" class="redirecttopage driver_pending"><span>' + val
                        .status + '</span></td>';

                } else if (val.status == 'Driver Rejected') {
                    html = html + '<td data-url="' + route + '" class="redirecttopage driver_rejected"><span>' + val
                        .status + '</span></td>';

                } else if (val.status == 'Order Shipped') {
                    html = html + '<td data-url="' + route + '" class="redirecttopage order_shipped"><span>' + val
                        .status + '</span></td>';

                } else if (val.status == 'In Transit') {
                    html = html + '<td data-url="' + route + '" class="redirecttopage in_transit"><span>' + val
                        .status + '</span></td>';

                } else if (val.status == 'Order Completed') {
                    html = html + '<td data-url="' + route + '" class="redirecttopage order_completed"><span>' + val
                        .status + '</span></td>';
        ...
        function buildParcelTotal(snapshotsProducts) {
            ...
            if (snapshotsProducts.hasOwnProperty('taxSetting') && snapshotsProducts.taxSetting != '' && snapshotsProducts
                .taxSetting != null) {
        ...
        function setVisitors() {
            ...
            const data = {
                ...
                datasets: [{
                    data: [jQuery("#order_count").text(), jQuery("#users_count").text(), jQuery("#driver_count")
                        .text()
                    ],
                    backgroundColor: [
                        '#FFAB2E',

...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            db.collection('vendor_orders').where('status', 'in', ["Order Placed", "Order Accepted", "Order Shipped",
                "Order Completed", "Order Rejected", "Driver Rejected"
            ]).where('vendor.section_id', '==', active_id).get().then(
                (snapshot) => {
            ...
            db.collection('vendor_orders').where('status', 'in', ["Order Accepted"]).where('vendor.section_id',
                '==', active_id).get().then(
                (snapshot) => {
            ...
            db.collection('vendor_orders').where('status', 'in', ["Order Shipped"]).where('vendor.section_id', '==',
                active_id).get().then(
                (snapshot) => {
        ...
        function buildDriverHTML(snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                html = html + '<td data-url="' + driverroute + '" class="redirecttopage">' + val.firstName + ' ' +
                    val.lastName + '</td>';
            ...
            const data = {
                ...
                datasets: [{
                    data: [jQuery("#vendor_count").text(), jQuery("#order_count").text(), jQuery(
                        "#product_count").text(), jQuery("#users_count").text()],
                    backgroundColor: [
                        '#218be1',
                        '#B1DB6F',
                        '#7360ed',

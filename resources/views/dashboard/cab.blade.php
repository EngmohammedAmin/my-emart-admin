    ...
    <script>
        ...
        async function getSectionData() {
            ...
            if (data.rideType == "intercity" || data.rideType == "both") {
                ...
                db.collection('users').where('rideType', 'in', ['intercity', 'both']).where("role", "==", "driver")
                    .where('serviceType', '==', active_type).get().then((snapshot) => {
                        jQuery("#driver_count_intercity").empty();
                        jQuery("#driver_count_intercity").append("{{ trans('lang.intercity') }} / " + snapshot.docs
                            .length);
                    });

                db.collection('rides').where('rideType', '==', 'intercity').where('status', 'in', ["Order Placed",
                    "Order Accepted", "Order Rejected", "Order Completed", "In Transit"
                ]).get().then((snapshot) => {
                    jQuery("#total_rides_intercity").empty();
                    jQuery("#total_rides_intercity").text("{{ trans('lang.intercity') }} / " + snapshot.docs
                        .length);
                });

                db.collection('rides').where('rideType', '==', 'intercity').where('status', 'in', ["Order Placed",
                    "Order Accepted", "In Transit"
                ]).get().then((snapshot) => {
                    jQuery("#confirmed_count_intercity").empty();
                    jQuery("#confirmed_count_intercity").text("{{ trans('lang.intercity') }} / " + snapshot
                        .docs.length);
                });

                db.collection('rides').where('rideType', '==', 'intercity').where('status', 'in', ["Order Completed"])
                    .get().then(
                        (snapshot) => {
                            jQuery("#completed_count_intercity").empty();
                            jQuery("#completed_count_intercity").text("{{ trans('lang.intercity') }} / " + snapshot
                                .docs.length);
                        });

                db.collection('rides').where('rideType', '==', 'intercity').where('status', 'in', ["Order Rejected"])
                    .get().then(
                        (snapshot) => {
                            jQuery("#canceled_count_intercity").empty();
                            jQuery("#canceled_count_intercity").text("{{ trans('lang.intercity') }} / " + snapshot.docs
                                .length);
                        });
        ...
        $(document).ready(function() {
            ...
            db.collection('users').where("role", "==", "driver").where('serviceType', '==', active_type).get().then(
                (snapshot) => {
                    jQuery("#driver_count").empty();
                    jQuery("#driver_count").append(snapshot.docs.length);
                });

            db.collection('rides').where('status', 'in', ["Order Placed", "Order Accepted", "Order Rejected",
                "Order Completed", "In Transit"
            ]).get().then((snapshot) => {
                jQuery("#total_rides").empty();
                jQuery("#total_rides").text(snapshot.docs.length);
            });

            db.collection('rides').where('status', 'in', ["Order Placed", "Order Accepted", "In Transit"]).get()
                .then(
                    (snapshot) => {
            ...
            ref.orderBy('createdAt', 'desc').where('status', 'in', ["Order Placed", "Order Accepted",
                "Driver Pending", "Driver Accepted", "Order Shipped", "In Transit"
            ]).limit(inx).get().then((snapshots) => {
                html = '';
            ...
            ref.where('role', '==', 'driver').where('serviceType', '==', active_type).orderBy('orderCompleted',
                'desc').limit(inx).get().then((snapshots) => {
                html = '';
        ...
        async function getTotalEarnings() {
            ...
            await db.collection('rides').where('status', 'in', ["Order Completed"]).get().then(async function(
                orderSnapshots) {
                ...
                paymentData.forEach((order) => {
                    ...
                    if (orderData.deliveryCharge != undefined && orderData.deliveryCharge != "" &&
                        orderData.deliveryCharge > 0) {
                        price = price + parseFloat(orderData.deliveryCharge);
                    }

                    if (orderData.adminCommission != undefined && orderData.adminCommissionType !=
                        undefined && orderData.adminCommission > 0 && price > 0) {
                        var commission = 0;
                    ...
                    } else if (orderData.adminCommission != undefined && orderData.adminCommission >
                        0 && price > 0) {
                    ...
                    try {
                        ...
                        db.collection('vendor_orders').doc(orderData.id).update({
                            'createdAt': dates
                        }).then(() => {
        ...
        async function getTotalEarningsIntercity() {
            ...
            await db.collection('rides').where('rideType', '==', 'intercity').where('status', 'in', ["Order Completed"])
        ...
        function buildHTML(snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                if (val.photo == '') {
                    ...
                    html = html +
                        '<td class="text-center"><img class="img-circle img-size-32 mr-2" style="width:60px;height:60px;" src="' +
                        placeholderImage + '" alt="image"></td>';
                } else {
                    html = html +
                        '<td class="text-center"><img class="img-circle img-size-32 mr-2" style="width:60px;height:60px;" src="' +
                        val.photo + '" alt="image"></td>';
                }
        ...
        function buildDriverHTML(snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                if (val.profilePictureURL == '') {
                    ...
                    html = html +
                        '<td class="text-center"><img class="img-circle img-size-32 mr-2" style="width:60px;height:60px;" src="' +
                        val.profilePictureURL + '" alt="image"></td>';
                }
                // html=html+'<td>'+val.firstName+' '+val.lastName+'</td>';
                html = html + '<td data-url="' + driverView + '" class="redirecttopage">' + val.firstName + ' ' +
                    val.lastName + '</td>';
                html = html + '<td data-url="' + driverView + '" class="redirecttopage">' + val.orderCompleted +
                    '</td>';
                html = html + '<td data-url="' + driverroute +
                    '" class="redirecttopage"><span class="fa fa-edit"></span></td>';
                html = html + '</tr>';
                count++;
            });
            return html;
        ...
        function buildRidesHTML(snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                if (val.hasOwnProperty("author")) {
                    html = html + '<td data-url="' + user_view + '" class="redirecttopage">' + val.author
                        .firstName + '</td>';
                } else {
                    html = html + '<td></td>';
                }
                ...
                if (val.hasOwnProperty("driver")) {
                    ...
                    html = html + '<td data-url="' + driverView + '" class="redirecttopage">' + val.driver
                        .firstName + '</td>';
                } else {
                    html = html + '<td></td>';
                }
                    ...
                    tooltips: {
                        ...
                        callbacks: {
                            label: function(tooltipItems, data) {
                                ...
                                if (currencyAtRight) {
                                    return (data.datasets[0].data[tooltipItems.index]).toFixed(decimal_degits) +
                                        currentCurrency;

                                } else {
                                    return currentCurrency + (data.datasets[0].data[tooltipItems.index])
        ...
        function buildHTMLSections(sectionsSnapshot) {
            ...
            html = html + '<div class="cat-item px-2 py-1 select_section ' + active_section + '"><a href="' + all_route +
                '" class="bg-white d-block p-2 text-center shadow-sm cat-link"><img alt="#" src="' + img_url +
                '" class="img-fluid mb-2"><p class="m-0 small">All</p></a></div>';
            ...
            alldata.forEach((listval) => {
                ...
                html = html + '<div class="cat-item px-2 py-1 select_section ' + active_section + '"><a href="' +
                    section_route +
                    '" class="bg-white d-block p-2 text-center shadow-sm cat-link"><img alt="#" src="' + photo +
        ...
        function setVisitors() {
            ...
            const data = {
                ...
                datasets: [{
                    data: [jQuery("#total_rides").text(), jQuery("#users_count").text(), jQuery("#driver_count")
                        .text(), jQuery("#confirmed_count").text(), jQuery("#completed_count").text(),
        ...
        function setCommision() {
            ...
            const data = {
                ...
                datasets: [{
                    data: [jQuery("#earnings_count").text().replace(currentCurrency, ""), jQuery(
                        "#admincommission_count").text().replace(currentCurrency, "")],
                    backgroundColor: [
                        '#feb84d',
                        '#9b77f8',
                        '#fe95d3'
                            ...
                            label: function(tooltipItems, data) {
                                if (currencyAtRight) {
                                    return data.labels[tooltipItems.index] + ': ' + (data.datasets[0].data[
                                        tooltipItems.index]) + currentCurrency;

                                } else {
                                    return data.labels[tooltipItems.index] + ': ' + currentCurrency + (data
                                        .datasets[0].data[tooltipItems.index]);

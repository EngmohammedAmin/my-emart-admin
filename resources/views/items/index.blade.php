                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-3 sectionDiv">
                                        <select id="section_id" class="form-control allModules" style="width:100%"
                                            ...
                                            <option value="">{{ trans('lang.select') }}
...
@section('scripts')
    <script type="text/javascript">
        ...
        if (brandID != '' && brandID != undefined) {
            if (section_id != '') {
                var ref = database.collection('vendor_products').where('brandID', '==', brandID).where('section_id', '==',
                    section_id);
            } else {
            ...
            if (section_id != '') {
                var ref = database.collection('vendor_products').where('categoryID', '==', categoryID).where('section_id',
                    '==', section_id);
            } else {
            ...
            $(document.body).on('change', '#selected_search', function() {
                ...
                if (jQuery(this).val() == 'brand') {
                    database.collection('brands').get().then(async function(snapshots) {
                        snapshots.docs.forEach((listval) => {
                            var data = listval.data();
                            $('#brand_search_dropdown').append($("<option></option")
                    ...
                    var section_id = getCookie('section_id');
                    if (section_id != '') {
                        var ref_category = database.collection('vendor_categories').where('section_id',
                            '==', section_id);
                    } else {
                    ...
                    ref_category.get().then(async function(snapshots) {
                        snapshots.docs.forEach((listval) => {
                            var data = listval.data();
                            $('#category_search_dropdown').append($("<option></option")
                                .attr("value", data.id).text(data.title));
        ...
        async function getListData(val) {
            ...
            if (val.photo != '') {
                ...
                html = html + '<td><img class="rounded" style="width:50px" src="' + placeholderImage +
                    '" alt="image"></td>';
            }
            ...
            if (val.hasOwnProperty('disPrice') && val.disPrice != '' && val.disPrice != '0') {
                if (currencyAtRight) {
                    ...
                    html = html + '<td>' + parseFloat(val.disPrice).toFixed(decimal_degits) + '' + currentCurrency +
                        '  <s>' + parseFloat(val.price).toFixed(decimal_degits) + '' + currentCurrency + '</s></td>';
                } else {
                    html = html + '<td>' + '' + currentCurrency + parseFloat(val.disPrice).toFixed(decimal_degits) +
                        '  <s>' + currentCurrency + '' + parseFloat(val.price).toFixed(decimal_degits) + '</s> </td>';
                }

            } else {

                if (currencyAtRight) {
                    html = html + '<td>' + parseFloat(val.price).toFixed(decimal_degits) + '' + currentCurrency +
                        '</td>';
                } else {
                    html = html + '<td>' + currentCurrency + '' + parseFloat(val.price).toFixed(decimal_degits) +
                        '</td>';
                }
            }
            ...
            html = html + '<td class="action-btn"><a href="' + route1 +
                '" class="link-td"><i class="fa fa-edit"></i></a><a id="' + val.id +
                '" name="item-delete" href="javascript:void(0)" class="link-td do_not_delete"><i class="fa fa-trash"></i></a></td>';
        ...
        function prev() {
            ...
            if (end != undefined || end != null) {
                ...
                if (jQuery("#selected_search").val() == 'title' && jQuery("#search").val().trim() != '') {
                    ...
                    listener = ref.orderBy('name').limit(pagesize).startAt(jQuery("#search").val()).endAt(jQuery("#search")
                        .val() + '\uf8ff').startAt(end).get();
                    ...
                    if (jQuery("#brand_search_dropdown").val() == "All") {
                        ...
                        listener = ref.orderBy('brandID').limit(pagesize).startAt(jQuery("#brand_search_dropdown").val())
                            .endAt(jQuery("#brand_search_dropdown").val() + '\uf8ff').startAt(end).get();
                ...
                } else if (jQuery("#selected_search").val() == 'category' && jQuery("#category_search_dropdown").val()
                    .trim() != '') {

                    if (jQuery("#category_search_dropdown").val() == "All") {
                        wherequery = ref.limit(pagesize).startAt(end).get();
                    } else {
                        wherequery = ref.orderBy('categoryID').limit(pagesize).startAt(jQuery("#category_search_dropdown")
                            .val()).endAt(jQuery("#category_search_dropdown").val() + '\uf8ff').startAt(end).get();
                    ...
                    database.collection('vendors').where('title', '==', title).get().then(async function(snapshots) {
                        ...
                        if (snapshots.docs.length > 0) {
                            ...
                            wherequery = ref.orderBy('vendorID').limit(pagesize).startAt(storedata.id).endAt(
                                storedata.id + '\uf8ff').startAt(end).get();
            ...
            if (start != undefined || start != null) {
                ...
                if (jQuery("#selected_search").val() == 'title' && jQuery("#search").val().trim() != '') {
                    ...
                    if (jQuery("#brand_search_dropdown").val() == "All") {
                        ...
                        listener = ref.orderBy('brandID').limit(pagesize).startAt(jQuery("#brand_search_dropdown").val())
                            .endAt(jQuery("#brand_search_dropdown").val() + '\uf8ff').startAfter(start).get();
                    ...
                    if (jQuery("#category_search_dropdown").val() == "All") {
                        ...
                        wherequery = ref.orderBy('categoryID').limit(pagesize).startAt(jQuery("#category_search_dropdown")
                            .val()).endAt(jQuery("#category_search_dropdown").val() + '\uf8ff').startAfter(start).get();
                    ...
                    database.collection('vendors').where('title', '==', title).get().then(async function(snapshots) {
                        ...
                        if (snapshots.docs.length > 0) {
                            ...
                            wherequery = ref.orderBy('vendorID').limit(pagesize).startAt(storedata.id).endAt(
                                storedata.id + '\uf8ff').startAfter(start).get();
        ...
        function searchtext() {
            ...
            if (jQuery("#selected_search").val() == 'title' && jQuery("#search").val().trim() != '') {
                ...
                wherequery = ref.orderBy('name').limit(pagesize).startAt(jQuery("#search").val()).endAt(jQuery("#search")
                    .val() + '\uf8ff').get();
                wherequery.then((snapshots) => {
                    html = '';
                    html = buildHTML(snapshots);
                ...
                if (jQuery("#brand_search_dropdown").val() == "All") {
                    ...
                    wherequery = ref.orderBy('brandID').limit(pagesize).startAt(jQuery("#brand_search_dropdown").val())
                        .endAt(jQuery("#brand_search_dropdown").val() + '\uf8ff').get();
                ...
                if (jQuery("#category_search_dropdown").val() == "All") {
                    ...
                    wherequery = ref.orderBy('categoryID').limit(pagesize).startAt(jQuery("#category_search_dropdown")
                    .val()).endAt(jQuery("#category_search_dropdown").val() + '\uf8ff').get();
